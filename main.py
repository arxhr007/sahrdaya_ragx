...
while True:
    ...
    # has_useful_link, not a bare URL match: a generic homepage in the answer used to
    # suppress this fallback even when the real document was retrievable.
    if query_likely_needs_links(user_input) and not has_useful_link(response):
        fallback_urls = extract_urls(context_str, limit=6)
        if not fallback_urls:
            fallback_urls = retrieve_supporting_urls(user_input, limit=6)
        if fallback_urls:
            response = response.rstrip() + "\n\n" + format_fallback_links(user_input, fallback_urls)
            response = harmonize_response_with_links(response, links_appended=True)
